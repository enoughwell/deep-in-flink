...
- **With Periodic Watermarks**
  ...
  ​	1） `AscendingTimestamps`

  

  ​	2）`BoundedOutOfOrderTimestamps`
...
- **With PreserveWatermark**

  不错处理，保持数据流中的Watermark原状。
...
![1564449799548](images/1564449799548.png)
...
Flink  Metrics指任务在flink集群中运行过程中的各项指标，包括机器系统指标：Hostname，CPU，Memory，Thread，GC，NetWork，IO，JVM 和 任务运行组件（JobManager，TaskManager，Job，Task，Operater）的相关指标。

Flink的Metric模块有两个作用：

1. 实时采集metrics的数据供flink UI进行数据展示，用户可以在页面上看到自己提交任务的状态，延迟等信息。
2. 对外提供metrics收集接口，用户可以将整个fllink集群的metrics通过MetricsReport上报至第三方系统进行存储，展示和监控。

第2种对IT规模很大的公司很有用（如互联网公司、大型金融机构、运营商），一般他们的集群规模比较大，Flink UI的展示和易用性不够，并且大型机构一般都有集中的运维管理系统，所以就通过metrics上报的方式与现有的IT运维系统进行集成，利用现有工具进行dashboard展示，同时存储下来的metrics可以用于监控报警，更进一步来说，可以用历史数据进行数据挖掘产生更大的价值，例如分析扩容时机等。

Flink原生的提供JMX、Graphite、InfluxDB、Prometheus 、PrometheusPushGateway、StatsD、Datadog、Slf4j，共8种Reporter，用户可以直接配置使用。
...
Flink能够跟踪时间在整个系统中处理的延迟，该功能默认是关闭的。可以在`ExecutionConfig`中配置`latencyTrackingInterval `(毫秒)开启延迟跟踪功能，`latencyTrackingInterval `的值要大于0。 ro

在`latencyTrackingInterval`，在Source中周期性发送`LatencyMarker`（延迟标记），延迟标记中包含一个在Source端发出事件的时间戳。延迟标记在处理过程中，被当做一般的事件进行，如果存在拥堵依然需要排队，延迟标记会体现数据等待处理的延迟时间。

> **注意**：
>
> 延迟标记并不会体现数据处理的延迟，因为在Operator中并不会处理，直接就发送给下游。例如对于窗口运算，延迟标记会直接绕过窗口，而一般的元素处理则需要等待窗口触发。只有Operator无法接受新的数据，例如checkpoint窗口对齐、反压等情况，导致数据被缓冲等待的延迟会体现在延迟标记中。

延迟标记用来跟踪DAG图中从Source端到下游Operator之间延迟分布，延迟分布最终作为Histogram类型的指标。指标的粒度使用[metrics-latency-interval](http://ci.apache.org/projects/flink/flink-docs-release-1.8/ops/config.html#metrics-latency-interval)参数配置。最细粒度是subtask，启用subtask级别的延迟跟踪，可能会导致产生大量的Histogram计算。

目前，Flink默认认为集群中所有机器的时钟是同步的。所以在实际的生产环境中，最好保证集群的时钟同步，以免延迟跟踪的结果是错的。

> <font color=red>**特别注意**</font>
>
> 启用延迟跟踪可能严重的影响集群的性能（特别是以subtask为粒度）。强烈建议只在必要时使用，例如debug排错。
...
![1564390832992](images/1564390832992.png)
...
为了便于对Metric进行方便的管理和区分，可以对Metric进行分类，MetricGroup就是用来实现这个功能的，MetricGroup还可以有子MetricGroup作为子分类。

> 实际上MetricGroup是一个树数据结构。

`AbstractMetricGroup`是大部分MetricGroup的抽闲类，提供了添加Metric和Group的关键方法。

`ProxyMetricGroup`比较特别，是一个代理类，主要的功能是提供将调用代理到父MetricGroup中。

**继承关系**

![1564394074107](images/1564394074107.png)



### MetricReporter

Reporter用来将Flink Metric暴露给外部的系统。

Reporter可以通过以下两种方式初始化：

1. **使用反射**

   使用反射初始化，要求Reporter的实现类，必须提供public的无参构造函数，并且不能是抽象类。

2. **使用MetricReporterFactory（推荐方式）**

   使用该方式，没有什么限制。

> 没有必要，也不推荐同时支持两种初始化方式。

![1564394342010](images/1564394342010.png)



### MetricRegistry

MetricRegistry追踪所有已注册的Metric，是MetricGroup和MetricReporter之间的纽带。

当在MetricGroup添加Metric的时候，其调用过程如下所示：

```java
MetricGroup.addMetric -> MetricRegistry.addMetric -> MetricReporter.addMetric
```



![1564394475048](images/1564394475048.png)
...
![1564394768856](images/1564394768856.png)



如上图所示，`MetriQueryService`继承了`RpcEndpoint`，提供了堆外提供RPC调用的能力，实现了`MetricQueryServiceGateway`接口，提供了指标查询的方法。

`MetricQueryService`与其他Flink组件相比，MetricQueryService在一个独立的线程池中执行，线程池中的线程池优先级比较低，启动时使用低优先级线程。参考文章《[Akka dispatcher with low priority threads](http://scalaakka.blogspot.com/2015/07/akka-dispatcher-with-low-priority.html)》

`MetricQueryService`中，使用KV的形式，按照指标类型来保存注册到Flink中的所有指标，如下所示：
...
`MetricQueryService`底层使用Akka的Actor实现，能够响应如下事件：

- **添加metric**

  调用`addMetric(String metricName, Metric metric, AbstractMetricGroup group)`方法

- **删除metric**

  调用`removeMetric(Metric metric)`方法

- **metric查询**

  调用`queryMetrics(Time timeout)`方法
...
Flink 使用Kerberos进行安全认证的目标如下：

1. 使flink集群中运行的job通过flink connector可以安全的访问数据，如读写kafka。
2. 能够与Zookeeper之间保持认证（需配置SASL）
3. 能够与Hadoop体系的组件保持认证，如HDFS、HBase等

在生产环境中，流计算Job一般被看作持续执行的，可能会持续运行几个月，在job运行的整个过程中，需要持续的安全认证，确保数据安全。与Hadoop的代理token和ticket缓存不同，Kerberos的keytab不会过期。

在当前的Flink实现中，支持在Flink集群（包含其中的组件JobManager、TaskManager和运行的Job）中配置keytab凭证或者Hadoop的代理token。

> 同一个Flink集群的Job共享凭证，如果想为某个Job配置特殊的keytabl 凭证，需要另起一个集群。在YARN、MESOS、K8s这些环境中，可以比较容易的启动多个Flink集群实例。

### Flink 安全模块的工作原理

Flink应用访问外部数据源，例如Kafka、Hdfs、Cassandra、Flume、Kinesis等，需要使用内置的或者第三方Connector，不同的Connector使用不同的安全认证机制，如Kerberos、SSL/TLS、用户名/密码。

目前Flink有限支持Kerberos协议，支持的Connector和服务如下：
...
> 注意
>
> 在Flink中可以为单独的服务或者Connector配置Kerberos，可以在方位Hadoop组件启用Kerberos的情况下，为Zookeeper单独配置，反之亦然。只有Kerberos 凭证是共享的。



Flink安全模块的继承体系如下：

![1564457124096](images/1564457124096.png)

`org.apache.flink.runtime.security.modules.SecurityModule`是安全模块的顶层接口，其他的安全认证的方式都实现了此接口。

### Hadoop安全模块

此模块使用 Hadoop `UserGroupInformation` (UGI)类，在访问Hadoop的这一端创建登录用户会话上下文，使用此会话上下文可以访问Hadoop的所有组件，包含：HDFS、HBase、YARN。

启用Hadoop安全后（在core-site.xml中配置），登录用户将使用配置的Kerberos凭证。否则将会使用本地启动Flink集群的操作系统账号的身份标识。

### JAAS安全模块

此模块为Flink集群提供动态的JAAS配置信息，使配置的Kerberos凭证可用于Zookeeper、Kafka、以及其他依赖于JAAS的服务或者Connector的安全认证。

> 注意
>
> 如果配置了静态的JAAS信息（配置文件），参见《[Java SE 文档](http://docs.oracle.com/javase/7/docs/technotes/guides/security/jgss/tutorials/LoginConfigFile.html)》。那么静态的配置信息会覆盖此模块中的动态JAAS信息。

### Zookeeper安全模块

此模块提供Flink端提供对ZooKeeper的安全认证的相关配置，即ZooKeeper服务名称（默认值：zookeeper）和JAAS登录上下文名称（默认值：Client）。
...
1. 在集群所有节点的Flink配置文件中添加安全配置的相关信息。
2. 确保集群所有节点的security.kerberos.login.keytab配置的目录中都有keytab文件。
3. 正常操作部署集群。
...
此模式下只需要配置一份Flink配置文件即可，YARN/Memos会自动将keytab文件复制到Flink容器中。

操作步骤：

1. 在连接YARN/Memos机器的Flink配置文件中添加安全配置的相关信息。
2. 确保security.kerberos.login.keytab配置的目录中有keytab文件
3. 正常操作部署集群。

**YARN专用方法kinit**

YARN模式下，没有keytab文件仍然可以部署安全的Flink集群。此模式下可以使用ticket缓存（使用kinit管理），省去创建keytab文件的繁琐过程和委托集群管理。在此模式下，Flink CLI申请Hadoop代理token（支持HDFS、HBASE）。

此模式的主要问题是生成的代理token会过期（一般不超过1周），对于长期运行的Flink集群不合适。
...
每一个使用Kerberos的组件都需要独自更新Kerberos票据授予票据（TGT）。Hadoop、Zookeeper、Kafka都能根据提供的keytab文件自动更新票据。使用代理token的时候，YARN自动更新Token（直到过期）。

## SSL

Flink提供了TSL/SSL的支持，能够认证和加密Flink集群内与外部交互的网络流量。

### 内部和外部连接

使用身份验证和加密保护计算机进程之间的网络连接时，Apache Flink可区分内部和外部连接。内部连接是指Flink进程之间建立的所有连接。 内部连接使用Flink自定义协议。 用户不能直接连接到内部连接端点。 外部/ REST连接是指从外部到Flink进程的所有连接。 外部连接包括用于启动和控制运行的Flink作业/应用程序的Web UI和REST命令，也包括Flink CLI与JobManager / Dispatcher的通信。

为了获得更大的灵活性，可以单独启用和配置内部和外部连接的安全性。

**内部、外部连接示意图**

![img](images/ssl_internal_external.svg)
...
- **控制消息**
  ...
  JobManager / TaskManager / Dispatcher / ResourceManager之间的RPC消息。

- **数据交换**

  TaskManagers之间的连接，用于在随机播放，广播，再分配等过程中交换数据。

- **Blob服务**

  lib和文件的分发。

有内部连接都经过SSL身份验证和加密。 连接使用相互身份验证，每个连接的服务器端和客户端都需要相互提供证书。

一般在部署Flink的时候，可以生成专用证书（可以是自签名的）。 外部与Flink交互的不需要内部通信证书，可以简单地添加到容器镜像或附加到YARN。

> 注意
>
> 由于内部连接使用共享证书进行相互身份验证，因此Flink可以跳过主机名验证。 基于容器的部署更加省事。



#### 外部/REST连接

所有外部连接都通过HTTP / REST接口暴露，由Web UI和CLI使用：

- Dispatcher通信以提交作业（会话群集）
- 与JobManager通信，监控和控制正在运行的作业/应用程序

可以将REST接口配置为SSL连接。 但是，默认情况下，服务器接受来自任意客户端的连接，REST接口不对客户端进行身份验证。

如果需要验证与REST端点的连接，可以通过配置启用简单的相互身份验证，但是一般建议部署Sidecar代理，将REST端点绑定到回环地址接口（或Kubernetes中的pod本地接口），启动一个REST代理，验证并将请求转发给Flink。 此类代理提供了多种身份验证选项，因此可以更好地集成到现有基础结构中。

> Envoy Proxy或带有MOD_AUTH的NGINX能够提供Sidecar代理的功能
